...
const { analyzeForWeb, analyzeConversationForWeb, analyzeConversationBaseForWeb, analyzeConversationTopicForWeb, analyzeConversationThemeForWeb, prepareDeterministicAnalysis } = require("./services/webAnalysis");
...
app.use((req, res, next) => {
  ...
  res.setHeader("Access-Control-Allow-Headers", "Authorization, Content-Type, X-Analysis-Mode, X-Locale, X-Reply-Goal, X-Reply-Style, X-Relationship-Status, X-Relationship-ID, X-User-Note, X-Analysis-Focus, X-RenAI-Device-ID, X-RenAI-Tracking-ID, X-RenAI-Fingerprint, X-RenAI-Request-ID");
...
app.post("/api/v1/anonymous/conversation-theme-reports",express.json({limit:"256kb"}),async(req,res)=>{
 ...
 const currentMetadata={theme_id:themeId,source_snapshot_id:sourceSnapshotId,source_version:sourceVersion,locale,content_version:THEME_CONTENT_VERSION,analysis_schema_version:THEME_ANALYSIS_SCHEMA_VERSION,final_copy_schema_version:THEME_FINAL_COPY_SCHEMA_VERSION,final_copy_prompt_version:THEME_FINAL_COPY_PROMPT_VERSION};
 const validAnalysis=value=>Boolean(value&&Array.isArray(value.stages)&&value.stages.length>=2&&typeof value.currentDirection==="string"&&Array.isArray(value.recentChanges));
 const validFinalCopy=value=>Boolean(value&&typeof value.headline==="string"&&typeof value.narrative==="string"&&Array.isArray(value.insights)&&value.insights.length===2&&value.insights.every(item=>item&&typeof item.id==="string"&&typeof item.title==="string"&&typeof item.body==="string"));
 const{data:cached}=await supabase.from("analyses").select("result").eq("user_id",authenticatedUser.id).eq("relationship_id",resolved.data.id).eq("status","completed").contains("input_metadata",currentMetadata).order("completed_at",{ascending:false}).limit(1).maybeSingle();
 if(cached?.result?.generation?.source==="real-ai"&&cached.result.analysisSchemaVersion===THEME_ANALYSIS_SCHEMA_VERSION&&validAnalysis(cached.result.analysis)&&cached.result.finalCopySchemaVersion===THEME_FINAL_COPY_SCHEMA_VERSION&&cached.result.finalCopyPromptVersion===THEME_FINAL_COPY_PROMPT_VERSION&&validFinalCopy(cached.result.finalCopy))return res.json({report:{...cached.result,generation:{...cached.result.generation,aiCalls:Number(cached.result.generation?.aiCalls||1),granularity:"theme",cached:true,source:"real-ai"}}});
 const messages=Array.isArray(req.body?.messages)?req.body.messages.filter(item=>["self","partner"].includes(item?.sender)&&String(item?.text||"").trim()).slice(-500).map(item=>({sender:item.sender,text:String(item.text).trim().slice(0,1500),timestamp:item.timestamp==null?null:String(item.timestamp).trim().slice(0,80)||null})):[];
 if(!messages.length)return res.status(400).json({error:"CONVERSATION_REQUIRED"});
 ...
 try{
  const usage=await usageService.check(authenticatedUser.id,"analysis");if(!usage.allowed)return res.status(429).json({error:"FAIR_USE_LIMIT_REACHED"});
  const{data:reusableRows}=await supabase.from("analyses").select("result").eq("user_id",authenticatedUser.id).eq("relationship_id",resolved.data.id).eq("status","completed").contains("input_metadata",{source:"premium_theme",theme_id:themeId,source_snapshot_id:sourceSnapshotId,source_version:sourceVersion,locale,analysis_schema_version:THEME_ANALYSIS_SCHEMA_VERSION}).order("completed_at",{ascending:false}).limit(10);
  const reusableAnalysis=(reusableRows||[]).map(row=>row?.result?.analysis).find(validAnalysis)||null;
  const{data:siblingRows}=await supabase.from("analyses").select("result").eq("user_id",authenticatedUser.id).eq("relationship_id",resolved.data.id).eq("status","completed").contains("input_metadata",{source:"premium_theme",source_snapshot_id:sourceSnapshotId,source_version:sourceVersion,locale,content_version:THEME_CONTENT_VERSION,analysis_schema_version:THEME_ANALYSIS_SCHEMA_VERSION,final_copy_schema_version:THEME_FINAL_COPY_SCHEMA_VERSION,final_copy_prompt_version:THEME_FINAL_COPY_PROMPT_VERSION}).order("completed_at",{ascending:false}).limit(10);
  const siblingFinalCopies={};for(const row of siblingRows||[]){const sibling=row?.result;if(sibling?.themeId&&sibling.themeId!==themeId&&validFinalCopy(sibling.finalCopy)&&!siblingFinalCopies[sibling.themeId])siblingFinalCopies[sibling.themeId]=sibling.finalCopy}
  const output=await analyzeConversationThemeForWeb({messages,selfName:String(req.body?.selfName||"").slice(0,80),partnerName:String(req.body?.partnerName||"").slice(0,80),locale,themeId,topics,analysisDraft:reusableAnalysis,siblingFinalCopies}),hookCopy={ja:{futarirashisa:"ふたりらしさ",feelings:"気持ち",future:"将来性"},"zh-TW":{futarirashisa:"兩人的樣子",feelings:"心意",future:"發展性"},en:{futarirashisa:"Type",feelings:"Feelings",future:"Potential"}}[locale][themeId];
  const report={relationshipId:resolved.data.id,themeId,sourceSnapshotId,sourceVersion,locale,schemaVersion:"diagnosis-theme-report-v3",analysisSchemaVersion:THEME_ANALYSIS_SCHEMA_VERSION,finalCopySchemaVersion:THEME_FINAL_COPY_SCHEMA_VERSION,finalCopyPromptVersion:THEME_FINAL_COPY_PROMPT_VERSION,generatedAt:new Date().toISOString(),hook:{title:hookCopy,judgment:output.finalCopy.headline,explanation:output.finalCopy.narrative,visual:themeId==="futarirashisa"?"mascot-type":themeId==="feelings"?"feelings-balance":"future-direction",position:2},analysis:output.analysis,finalCopy:output.finalCopy,generation:{aiCalls:1+(output.auxiliaryUsages?.length||0),analysisCalls:output.analysisReused?0:1,finalCopyCalls:1,analysisReused:output.analysisReused,granularity:"theme",cached:false,source:"real-ai"}};
  const{error}=await supabase.from("analyses").insert({user_id:authenticatedUser.id,relationship_id:resolved.data.id,mode:"analysis",status:"completed",title:`theme:${themeId}`,result:report,completed_at:new Date().toISOString(),input_metadata:{source:"premium_theme",...currentMetadata,message_count:messages.length}});if(error)throw new Error("THEME_REPORT_SAVE_FAILED");
  await usageService.recordSuccess(usage);await tracking.record({name:"ai_usage_completed",businessKey:`theme_report:${authenticatedUser.id}:${themeId}:${sourceSnapshotId}`,userId:authenticatedUser.id,source:"ai",properties:{...output.usage,module:"analysis",mode:"premium_theme",theme_id:themeId}}).catch(()=>null);for(const[index,auxiliary]of(output.auxiliaryUsages||[]).entries())await tracking.record({name:"ai_usage_completed",businessKey:`theme_report:${authenticatedUser.id}:${themeId}:${sourceSnapshotId}:aux:${index}`,userId:authenticatedUser.id,source:"ai",properties:{...auxiliary,module:"analysis",mode:"premium_theme_editorial_input",theme_id:themeId}}).catch(()=>null);
