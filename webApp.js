...
const THEME_FINAL_COPY_PROMPT_VERSION="diagnosis-theme-final-copy-prompt-v1";
const THEME_CONTENT_VERSION="diagnosis-theme-content-v5";
...
app.post("/api/v1/anonymous/conversation-theme-reports",express.json({limit:"256kb"}),async(req,res)=>{
 ...
 try{
  ...
  const output=await analyzeConversationThemeForWeb({messages,selfName:String(req.body?.selfName||"").slice(0,80),partnerName:String(req.body?.partnerName||"").slice(0,80),locale,themeId,topics,analysisDraft:reusableAnalysis}),hookCopy={ja:{futarirashisa:"ふたりらしさ",feelings:"気持ち",future:"将来性"},"zh-TW":{futarirashisa:"兩人的樣子",feelings:"心意",future:"發展性"},en:{futarirashisa:"Type",feelings:"Feelings",future:"Potential"}}[locale][themeId];
