...
app.post("/api/v1/anonymous/conversation-theme-reports",express.json({limit:"256kb"}),async(req,res)=>{
 ...
 const{data:cached}=await supabase.from("analyses").select("result").eq("user_id",authenticatedUser.id).eq("relationship_id",resolved.data.id).eq("status","completed").contains("input_metadata",{theme_id:themeId,source_snapshot_id:sourceSnapshotId,locale,content_version:"diagnosis-theme-content-v4"}).order("completed_at",{ascending:false}).limit(1).maybeSingle();
 if(cached?.result)return res.json({report:{...cached.result,generation:{aiCalls:Number(cached.result.generation?.aiCalls||1),granularity:"theme",cached:true,source:cached.result.generation?.source||"real-ai"}}});
 const messages=Array.isArray(req.body?.messages)?req.body.messages.filter(item=>["self","partner"].includes(item?.sender)&&String(item?.text||"").trim()).slice(-500).map(item=>({sender:item.sender,text:String(item.text).trim().slice(0,1500),timestamp:item.timestamp==null?null:String(item.timestamp).trim().slice(0,80)||null})):[];
 if(!messages.length)return res.status(400).json({error:"CONVERSATION_REQUIRED"});
 ...
 try{
  ...
  const output=await analyzeConversationThemeForWeb({messages,selfName:String(req.body?.selfName||"").slice(0,80),partnerName:String(req.body?.partnerName||"").slice(0,80),locale,themeId,topics}),hookCopy={ja:{futarirashisa:"ふたりらしさ",feelings:"気持ち",future:"将来性"},"zh-TW":{futarirashisa:"兩人的樣子",feelings:"心意",future:"發展性"},en:{futarirashisa:"Type",feelings:"Feelings",future:"Potential"}}[locale][themeId];
  const report={relationshipId:resolved.data.id,themeId,sourceSnapshotId,sourceVersion,locale,schemaVersion:"diagnosis-theme-report-v3",generatedAt:new Date().toISOString(),hook:{title:hookCopy,judgment:output.result.headline,explanation:output.result.summary,visual:themeId==="futarirashisa"?"mascot-type":themeId==="feelings"?"feelings-balance":"future-direction",position:2},content:output.result,generation:{aiCalls:1+(output.auxiliaryUsages?.length||0),granularity:"theme",cached:false,source:"real-ai"}};
  const{error}=await supabase.from("analyses").insert({user_id:authenticatedUser.id,relationship_id:resolved.data.id,mode:"analysis",status:"completed",title:`theme:${themeId}`,result:report,completed_at:new Date().toISOString(),input_metadata:{source:"premium_theme",content_version:"diagnosis-theme-content-v4",theme_id:themeId,source_snapshot_id:sourceSnapshotId,source_version:sourceVersion,locale,message_count:messages.length}});if(error)throw new Error("THEME_REPORT_SAVE_FAILED");
