...
async function analyzeConversationThemeForWeb({messages=[],selfName="",partnerName="",locale="ja",themeId,topics=[]}){
 ...
 const scrubEvidenceLanguage=value=>typeof value==="string"?value.replace(/[「」『』“”]/g,"").replace(/心の奥底/g,"本音").replace(/心の拠り所/g,"安心できる存在").replace(/かけがえのない/g,"とても大切な").replace(/心を満たしている/g,"安心させている").replace(/深い願い/g,"強い気持ち").replace(/守るべき大切な場所/g,"失いたくない関係").replace(/微かな疑念/g,"小さな迷い").replace(/潜んでいる/g,"残っている").replace(/感情の安全/g,"安心できる感覚").replace(/内面のバランス/g,"気持ちの落ち着き").replace(/心の安全/g,"安心").replace(/感情を大きく開く/g,"本音を見せる").replace(/心の壁/g,"ためらい").replace(/内面的な/g,"気持ちの").replace(/最大の障壁/g,"あと一歩進めない理由").replace(/決定的な転機/g,"次に関係が動くきっかけ"):value;
 const escapedNonMeetingInitiator=nonMeetingInitiator?nonMeetingInitiator.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"):null,falseMeetingInitiativePattern=escapedNonMeetingInitiator?new RegExp(`(?:${escapedNonMeetingInitiator}の(?:具体的な|積極的な|明確な)?誘い|${escapedNonMeetingInitiator}が[^。\\n]{0,45}(?:会う約束を(?:働きかけ|取り付け)|会う計画を働きかけ|誘った|主導権|イニシアチブ|関係を動かす原動力))`,`u`):null;
 const correctMeetingAttribution=value=>{if(typeof value!=="string")return value;const cleaned=value.replace(/[『』“”]/g,"").replace(/かたちている/g,"出ている");return falseMeetingInitiativePattern?cleaned.split(/(?<=。)/u).map(sentence=>falseMeetingInitiativePattern.test(sentence)?sentence.replace(nonMeetingInitiator,soleMeetingInitiator):sentence).join(""):cleaned};
 ...
 const verifierPrompt=`${roleLock}\n\nYou are RenAI's final factual and editorial author. Produce a fresh diagnosis-theme-report-v3 directly from SOURCE_RECORD, TRAJECTORY, and AUTHORITATIVE_BEHAVIOR_FACTS. Do not inherit or repair earlier prose. This is the final user-facing Japanese report.\n\nTHEME: ${promises[themeId]||promises.futarirashisa}\nPEOPLE: ${JSON.stringify(names)}\n\nHARD RULES:\n- AUTHORITATIVE_BEHAVIOR_FACTS overrides any ambiguous interpretation. Never name the recorded responder as the inviter.\n- Read SOURCE_RECORD chronologically and preserve the literal named actor of every behavior.\n- If person A asks availability and proposes meeting, A initiated. Person B accepting, confirming one-on-one, or naming a destination reciprocated; never describe B as the inviter or main driver for that event. If A first asks to meet again or checks a schedule after meeting, A followed through.\n- Do not flatten the responder into passive: accepting a one-on-one plan, naming a desired next destination, and continuing the plan are active reciprocation. Describe that investment accurately without transferring the original invitation.\n- Current state must reflect the latest meaningful behavioral reversal, not the average or an older quiet period.\n- Every card must begin from one concrete change or repeated behavior, then give one non-obvious answer to SO WHAT. The conclusion is the implication, not a restatement of the behavior.\n- TYPE is only the pair's distinctive relationship pattern and why it persists. FEELINGS is only ${names.partner||"the partner"}'s current emotional position toward ${names.self||"the self person"}, especially what recent renewed initiative says about intent. FUTURE is only the current direction, likely next movement, and concrete progress-versus-stall branch signals. Do not place a feelings card or future prediction inside TYPE; do not place TYPE taxonomy inside FUTURE.\n- FUTURE is not counseling. Do not prescribe honest communication, trust-building, distance adjustment, confession, or a generic further step by one person. Predict the next observable behavior and name what continuation versus renewed stalling would look like.\n- Never use analytic labels such as 誘い手, 応答手, 駆け引き型, 片方向, イニシアティブ型, 主導権の偏り, or report-style category names. Write ordinary Japanese a 20-year-old would use.\n- Do not describe ${names.self||"the self person"} as merely passive or claim they show no initiative when they actively accept, choose a destination, or continue a plan.\n- Do not make incidental objects, destinations, photos, or a third party the central relationship insight unless they truly change the trajectory.\n- Do not expose evidence lists, raw quotations, reasoning, methodology, or confidence. Never use 「 or 」.\n- Avoid repetitive abstract answers built on caution, distance, ambiguity, anxiety, trust, or communication.\n- Use natural contemporary Japanese and exactly three conclusions. Keep claims calibrated; no flattery or forced optimism. No sentence fragments, truncated endings, awkward noun stacking, or translated report language.`;
 const themeBoundary=themeId==="future"?`FUTURE BOUNDARY — ALL THREE CARDS MUST BE GENUINELY FUTURE-FACING. Select three distinct questions for this record: one about current trajectory or realistic romantic potential, one about the most likely next movement, and one about the concrete branch point, progress confirmation, renewed-stall signal, or what happens if nothing changes. Do not include a relationship-type card, an interaction-pattern card, or a card asking ${names.partner||"the partner"}'s current feelings. Do not reuse who sets the rhythm or how special one person feels as a main insight. Lead with the latest trajectory after cooling and renewed initiative.`:themeId==="feelings"?`FEELINGS BOUNDARY — ALL THREE CARDS MUST STAY ON ${names.partner||"the partner"}'s current emotional position: the kind of affection, emotional significance, what is held back, or what recent renewed initiative says emotionally. Do not predict long-term outcomes, give future branch points, or classify the pair's general relationship type.`:`TYPE BOUNDARY — ALL THREE CARDS MUST STAY ON the pair's shared identity and interaction pattern: what kind of pair they are, who sets the rhythm, what feels comfortable, and what distinctive dynamic keeps the current state in place. Do not forecast what happens next or deeply analyze one person's romantic feelings.`;
 const namingAndLanguageRule=`FINAL COPY POLISH — Keep the analysis and conclusions unchanged; polish only how they are expressed. USER-FACING NAMING: default to ふたり, 二人, 今のふたり, この関係, or ふたりの間. Use a person's name only when it is necessary to distinguish who initiated, responded, or felt something. After an actor is established, switch back to ふたり or この関係 instead of repeating either name. FEELINGS may name the focused person once where needed, but must not mechanically pair both names or repeatedly begin sentences with names. RELATIONSHIP LANGUAGE: write like a natural relationship reading, never an analysis note. Never use 表面的には, 決定打がないため, 進展の決め手に欠けている, or similarly clinical phrasing. Say the emotional relationship meaning in simple Japanese instead. HEADLINES: keep them short, memorable, and emotionally clear; target roughly 12–24 Japanese characters. A FUTURE headline must directly tell the reader where the relationship is heading now, not list actors, behaviors, caveats, or sound like an analyst summary. NATURAL JAPANESE: never repeat near-synonyms in one headline or paragraph, such as じれったい and 焦れったさ, or 慎重, ためらい, and 踏み込めない. Choose the single clearest expression.`;
 const themeAwareVerifierPrompt=`${verifierPrompt}\n\n${themeBoundary}\n${namingAndLanguageRule}`;
 const validateThemeBoundary=raw=>{const questions=raw.conclusions.map(item=>item.question||"");if(themeId==="future"&&questions.some(question=>/(?:関係性|関係の特徴|関係の本質|どんな二人|どんな関係|今の気持ち|現在の感情|感情の種類)/u.test(question)))throw new Error("AI_THEME_BOUNDARY_FUTURE");if(themeId==="feelings"&&questions.some(question=>/(?:この先|これから|今後|将来|分岐点|何もしなかったら)/u.test(question)))throw new Error("AI_THEME_BOUNDARY_FEELINGS");if(themeId==="futarirashisa"&&questions.some(question=>/(?:この先|これから|今後|将来|次に起こる|進展可能性)/u.test(question)))throw new Error("AI_THEME_BOUNDARY_TYPE")};
 const verified=await callStructured({prompt:themeAwareVerifierPrompt,task:JSON.stringify({sourceRecord:transcript,trajectory:trajectory.raw,authoritativeBehaviorFacts:{meetingInitiators:[...meetingInitiators],soleMeetingInitiator,nonMeetingInitiator,interpretation:soleMeetingInitiator?`${soleMeetingInitiator} initiated the meeting plans; ${nonMeetingInitiator} accepted or expressed a preference and must not be described as the inviter.`:"Meeting initiative was mutual or not uniquely attributable."}}),schema:themeReportSchema,maxTokens:6200,temperature:.15,reasoningEffort:"low",modelOverride:process.env.OPENAI_THEME_EDITOR_MODEL||"gpt-4.1",validate:raw=>{validateFinal(raw);validateThemeBoundary(raw)}});
