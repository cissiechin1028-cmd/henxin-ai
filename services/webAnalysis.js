...
async function analyzeConversationThemeForWeb({messages=[],selfName="",partnerName="",locale="ja",themeId,topics=[]}){
 ...
 const attributionSafeFinalPrompt=`${behavioralFinalPrompt}\n\nPERSON ATTRIBUTION CHECK: the trajectory actor names are authoritative. Never transfer initiative, cancellation, rebooking, follow-through, or future planning from one named person to the other. Preserve the exact actor attached to every concrete behavior in the input and verify all names before returning.\nOUTPUT PUNCTUATION: never use the characters 「 or 」 anywhere, including rhetorical labels, relationship names, headlines, questions, conclusions, or summaries. Express the idea directly without quotation marks.`;
