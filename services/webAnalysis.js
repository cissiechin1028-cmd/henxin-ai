...
async function analyzeConversationThemeForWeb({messages=[],selfName="",partnerName="",locale="ja",themeId,topics=[]}){
 ...
 const editorialInput={coreQuestion:draft.raw.coreQuestion,headline:draft.raw.headline,conclusions:draft.raw.conclusions.map(({id,question,conclusion})=>({id,question,conclusion}))};
 const validateFinal=raw=>{validateTheme(raw);const prose=topicStrings(raw).join("\n"),forbidden=["会話","メッセージ","やりとり","やり取り","返答","質問","提案","冗談","言葉の裏","態度","サイン","見られる","うかがえる","表れ"],matched=forbidden.filter(term=>prose.includes(term));if(matched.length)throw new Error(`AI_USER_FACING_EVIDENCE_LANGUAGE:${matched.join(",")}`)};
