...
async function analyzeConversationThemeForWeb({messages=[],selfName="",partnerName="",locale="ja",themeId,topics=[]}){
 ...
 const namingAndLanguageRule=`USER-FACING NAMING — Default to ふたり, 二人, 今のふたり, この関係, or ふたりの間. Use a person's name only when actor identity is genuinely necessary. FEELINGS may name the focused person, but never mechanically repeat both names. Do not repeatedly begin sentences with names. NATURAL JAPANESE — Never repeat near-synonyms in one headline or paragraph, such as じれったい and 焦れったさ, or 慎重, ためらい, and 踏み込めない. Choose the single clearest expression.`;
