...
async function analyzeConversationThemeForWeb({messages=[],selfName="",partnerName="",locale="ja",themeId,topics=[]}){
 ...
 const editorialPrompt=`${prompt}\n- Never mention chat, messages, questions, replies, frequency, observed wording, or behavioral evidence as the basis for a conclusion. Ban report phrases such as 見られる, うかがえる, 〜から分かる, 言動, and 表れ. Move one interpretive layer deeper: describe the emotional meaning, conflict, priority, or need itself.\n- CALIBRATE INTENSITY: emotional value comes from clarity, not flattery. Do not upgrade comfort into devotion, interest into love, or familiarity into irreplaceability. Avoid poetic inflation such as 心の奥底, 心の拠り所, かけがえのない, 心を満たしている, 深い願い, 守るべき大切な場所, or 微かな疑念が潜んでいる unless the record supports that exact intensity repeatedly and recently. Prefer plain, conversational Japanese.\n- MANDATORY TRAJECTORY PASS (internal only): before drafting, read the dated sequence as stages, not an average. Identify the earlier state, any cooling or withdrawal, the latest meaningful reversal, and the state after that reversal. BASELINE shows the longer pattern; RECENT carries more weight. The last meaningful direction overrides an older cold period unless a still later reversal cancels it. Renewed initiative, rebooking after a decline, restored contact, explicit desire to meet, concrete future plans, or moving an emotional topic toward an in-person discussion are relationship-changing events, not minor details. If they occur after cooling, the current state must be reconnecting or moving again—not still cooling or simply stagnant. Never cite these events in user-facing copy; express what the sequence means now.\n- CROSS-TAB CALIBRATION: infer one coherent current relationship state and keep every claim at that level. Feelings must not describe devotion or emotional refuge when the same state supports a stalled or low-commitment Future. A cautious Future can coexist with warmth, but the intensity must match.\n- SO-WHAT TEST: every conclusion must answer the paid question, not stop at an observation. Convert pace, comfort, distance, caution, or mismatch into what it means about the relationship's identity, ${names.partner||"the partner"}'s current position, or the likely next direction. Each deepening must add the practical emotional consequence: why this matters now, what it changes, or what uncertainty it resolves.\n- SCREENSHOT-WORTHY LINE: the headline must be one concise, memorable, plainspoken sentence that captures this relationship without poetry or slogans.\n- TYPE: make the shared relationship mechanism the subject. Each insight must cover a different axis: the unique comfort they create, who or what sets the emotional pace, and the distinctive pattern that explains the current distance. Do not answer hidden feelings or predict the future.\n- FEELINGS: make ${names.partner||"the partner"}'s current emotional sense of ${names.self||"the self person"} explicit, including what the latest reversal changes. The three insights must separately answer specialness versus comfort, why ${names.partner||"the partner"} has not stepped further, and what the renewed closeness means now. Do not repeat distance/caution three times.\n- FUTURE: the headline must directly say whether the relationship is stalled, reconnecting, moving, or fading now. The three insights must separately cover whether this romance is viable, the next likely turning point, and the likely destination if nothing else changes. Use conversational question titles such as a person would actually ask; never use bureaucratic labels such as 最大の障壁 or 決定的な転機. Advice may name the kind of move needed, but must not invent a specific event.\n- BAN THERAPY LANGUAGE: do not use 感情の安全, 内面のバランス, 心の安全, 感情を大きく開く, 心の壁, 内面的, or similarly abstract psychology language. Prefer ordinary Japanese a 20-year-old immediately understands.\n- Do not name concrete objects, plans, purchases, foods, places, timestamps, or individual exchanges. Do not present evidence or explain how RenAI reached the answer.\n- Use contemporary natural Japanese. Prefer direct lines such as ここで大事なのは or 今いちばん見るべきなのは when emphasis is useful. Never end explanations with 〜を意味しています, 〜を示しています, or 〜ことを表しています. Every conclusion and deepening must contribute a new answer.\n- Japanese quotation marks are allowed only around an abstract relationship label or emotional category, never around chat-derived wording.`;
 ...
 const validateFinal=raw=>{validateTheme(raw);const prose=topicStrings(raw).join("\n"),forbidden=["会話","メッセージ","やりとり","やり取り","返答","質問","提案","冗談","言葉の裏","態度","サイン","見られる","うかがえる","表れ","心の奥底","心の拠り所","かけがえのない","心を満たしている","深い願い","守るべき大切な場所","微かな疑念","潜んでいる","最大の障壁","決定的な転機","感情の安全","内面のバランス","心の安全","感情を大きく開く","心の壁","内面的","を意味しています","を示しています","ことを表しています"],matched=forbidden.filter(term=>prose.includes(term));if(matched.length)throw new Error(`AI_USER_FACING_EVIDENCE_LANGUAGE:${matched.join(",")}`)};
