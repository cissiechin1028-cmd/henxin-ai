...
async function analyzeConversationThemeForWeb({messages=[],selfName="",partnerName="",locale="ja",themeId,topics=[]}){
 ...
 const model=process.env.OPENAI_VISION_MODEL||"gpt-4.1-mini",window=selectAnalysisWindow(messages),selected=[...window.baseline.map(message=>({message,period:"BASELINE"})),...window.recent.map(message=>({message,period:"RECENT"}))],transcript=selected.map(({message,period},index)=>`${index+1}. [${period}] ${actorName(message)}: ${message.text} [${message.timestamp||"TIME UNKNOWN"}]`).join("\n");
