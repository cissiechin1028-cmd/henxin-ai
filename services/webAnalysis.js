...
async function analyzeConversationThemeForWeb({messages=[],selfName="",partnerName="",locale="ja",themeId,topics=[]}){
 ...
 const finalPrompt=`You are the final Japanese relationship editor for RenAI. Rewrite the supplied structured draft into the final user-facing Premium report. The draft is analysis material, not copy to preserve. Keep its grounded meaning, but remove every trace of evidence reporting. Never mention or imply chat, messages, questions, replies, wording, frequency, observations, signs, examples, or how the answer was inferred. Never cite a concrete exchange, object, plan, place, food, purchase, or timestamp. Write only the emotional meaning itself.\n\nPEOPLE: ${JSON.stringify(names)}\nTHEME: ${promises[themeId]||promises.futarirashisa}\nUse the actual names naturally and never use database-actor language. Keep one emotionally exact headline, one cohesive substantial summary, and exactly three distinct conclusions. Each deepening must explain what the answer means emotionally for this relationship, not provide proof. TYPE stays on shared identity/pattern; FEELINGS stays on ${names.partner||"the partner"}'s inner emotional state toward ${names.self||"the self person"}; FUTURE stays on trajectory, turning point, and likely destination. Use contemporary natural Japanese and do not add optimism, motivation, or certainty unsupported by the draft.`;
 const final=await callStructured({prompt:finalPrompt,task:JSON.stringify(draft.raw),schema:themeReportSchema,maxTokens:6200,temperature:.3,reasoningEffort:"low",validate:validateTheme});
