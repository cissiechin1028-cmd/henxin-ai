...
function registerPremiumDiagnosisRoute(app,{express,supabase,resolveRelationship,usageService,tracking}){
 ...
 async failed(id){const{error}=await supabase.from('analyses').update({status:'failed',error_code:'PREMIUM_FINAL_COPY_FAILED'}).eq('id',id);if(error)throw new Error('PREMIUM_FAILURE_SAVE_FAILED')}
 };
 ...
 const report=await pipeline.generate({userId:user.id,relationshipId:relationship.data.id,sourceSnapshotId,sourceVersion,locale},messages,{self:String(req.body?.selfName||'').slice(0,80),partner:String(req.body?.partnerName||'').slice(0,80)});
